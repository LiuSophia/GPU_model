...
int launchVOKernel(std::vector<Keep_Feature> matchfeature , std::vector<Keep_Feature> map_feature,float *CameraPose,int *max_Consensus,double l_u0,double l_v0,double l_fu,double l_fv)
{
	...
	float *parm/*, *CameraPose*/,*GPU_WL,*GPU_Rs;
	int *ransac_match_max,*index_num,*index_max,*ransac_match_num_all;//符合共識集合的數量
	...
	GPU_WL=(float*)malloc(3 *sol_num* sizeof(float));
	GPU_Rs=(float*)malloc(9 *sol_num* sizeof(float));
	ransac_match_num_all=(int*)malloc(sol_num* sizeof(int));
	...
	cudaMemcpy(ransac_match_max, dev_ransac_match_max,  sizeof(int), cudaMemcpyDeviceToHost);
	...
	return ransac_match_max[0];
	
}

void launchVOKernel_dub(std::vector<Keep_Feature> matchfeature , std::vector<Keep_Feature> map_feature,std::vector<Ransac_Pos> &ransac_match,double *CameraPose,double l_u0,double l_v0,double l_fu,double l_fv)
{
	int thredperblock = NT;
	int sol_num=(matchfeature.size()*(matchfeature.size()-1)*(matchfeature.size()-2))/6; //n取3
	int blocknum=(sol_num+thredperblock-1)/thredperblock;
	...
	int *P3P_NO;//建model的特徵點編號
	double *parm/*, *CameraPose*/,*GPU_WL,*GPU_Rs;
	int *ransac_match_max,*index_num,*index_max,*ransac_match_num_all;//符合共識集合的數量
	matchfeature_NO = (int*)malloc(matchfeature.size()* sizeof(int));
	map_feature_NO = (int*)malloc(map_feature.size()* sizeof(int));
	...
	index_num=(int*)malloc(sol_num* sizeof(int));
	index_max=(int*)malloc(blocknum* sizeof(int));
	GPU_WL=(double*)malloc(3 *sol_num* sizeof(double));
	GPU_Rs=(double*)malloc(9 *sol_num* sizeof(double));
	ransac_match_num_all=(int*)malloc(sol_num* sizeof(int));
