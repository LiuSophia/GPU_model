...
//	virtual ~RANSAC();
	...
	vector<keepfeature> matchfeature;
	vector<keepfeature> map_feature;
	//vector<Keep_Feature> last_map_feature;
	//P3PSolver p3p_link;

	void Run_RANSAC( vector<keepfeature> matchfeature , vector<keepfeature> map_feature,vector<Ransac_Pos> &ransac_match,double *,double *,int *,double l_u0,double l_v0,double l_fu,double l_fv);
	int match_number ;
	int ransac_time;
