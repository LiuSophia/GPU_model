...
RANSAC_ho ransac;
P3PSolver p3p_link;
void RANSAC_ho::Run_RANSAC( vector<keepfeature> matchfeature , vector<keepfeature> map_feature,vector<Ransac_Pos> &ransac_match,double *CPU_WL,double *CPU_Rs,int *CPU_ransac_match,double l_u0,double l_v0,double l_fu,double l_fv)
{
	bool again = true; 
	double range = 25.0;
	int match_number ;
	...
	for(int g=0; again && g<matchfeature.size()-2 ; g++)
	{
		for(int g_two=g+1 ; again && g_two< matchfeature.size()-1 ; g_two++ )
		{
			for(int g_three=g_two+1 ; again && g_three< matchfeature.size() ; g_three++ )
			{
				...
				ptw[0] = matchfeature[g].hx;
				ptw[1] = matchfeature[g].hy;
				ptw[2] = matchfeature[g].hz;
				pti[0] = matchfeature[g].l_ix;
 				pti[1] = matchfeature[g].l_iy;

				ptw[3] = matchfeature[g_two].hx;
				ptw[4] = matchfeature[g_two].hy;
				ptw[5] = matchfeature[g_two].hz;
				pti[2] = matchfeature[g_two].l_ix;
 				pti[3] = matchfeature[g_two].l_iy;

				ptw[6] = matchfeature[g_three].hx;
				ptw[7] = matchfeature[g_three].hy;
				ptw[8] = matchfeature[g_three].hz;
				pti[4] = matchfeature[g_three].l_ix;
 				pti[5] = matchfeature[g_three].l_iy;
