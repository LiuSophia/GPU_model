...
void InitialP3P()
{
	...
	fstream para("Paratxt//parameter_DataSize.txt",ios::in );
...
int main(int argc, char* argv[])
{
	...
	for(int i=0;i<sol_num;i++)
	{
		if(CPU_ransac_match[i]==maxsetsize)
		{
	...
	{
		printf("%.13f\t",CPU_WL[consensus_set[i]*3]);
		printf("%.13f\t",CPU_WL[consensus_set[i]*3+1]);
		printf("%.13f\n",CPU_WL[consensus_set[i]*3+2]);
		printf("%.13f\t",atan2(CPU_Rs[consensus_set[i]*9+5], CPU_Rs[consensus_set[i]*9+8])* 180 / PI + 90);
		printf("%.13f\t",asin(-CPU_Rs[consensus_set[i]*9+2])* 180 / PI );
		printf("%.13f\n",atan2(CPU_Rs[consensus_set[i]*9+1], CPU_Rs[consensus_set[i]*9])* 180 / PI );
	}
	...
	/* for(int i=0;i<sol_num;i++)
	{
		if(sqrt(CPU_WL[i*3]-GPU_WL_dub[i*3])*(CPU_WL[i*3]-GPU_WL_dub[i*3])>0.000000000001)
		{
			gpu_WL_d_t++;
			cout<<i<<endl;
			cout<<"CPU_WL"<<setw(5)<<CPU_WL[i*3]<<setw(15)<<CPU_WL[i*3+1]<<setw(15)<<CPU_WL[i*3+2]<<endl;
